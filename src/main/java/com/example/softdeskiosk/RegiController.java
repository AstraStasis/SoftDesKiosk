...
public class RegiController implements Initializable {
    private Kiosk kiosk;
    ...
    @FXML private void handlePrintIntentLetter() {
        ...
        if (yearLevel == null || semester == null || purpose == null) {
            System.err.println("Please select year level, semester, and purpose.");
            return;
        }

        StudentInfo info = students.get(key);
        VBox letter = buildIntentLetter(info, yearLevel, semester, purpose);
        PrinterJob job = PrinterJob.createPrinterJob();
        if (job != null && job.printPage(letter)) job.endJob();
    }

    private VBox buildIntentLetter(StudentInfo info,
                                   String yearLevel,
                                   String semester,
                                   String purpose) {
        ...
        box.getChildren().addAll(
                new Text("Ms. SFAU Head"),
                new Text("Scholarship and Financial Assistance Unit"),
        ...
        Text bodyText = new Text(
                "I would like to request a registration form for " +
